// Pressure Jacobi solver - iterative solver for pressure Poisson equation
// Ports js/05-fluid-sim.js pressureFrag to WGSL compute shader
...
fn pressure_jacobi(@builtin(global_invocation_id) id: vec3<u32>) {
    ...
    let div = textureSampleLevel(divergence_texture, point_sampler, uv, 0.0).x;
    
    // Jacobi iteration: p = (pL + pR + pB + pT - divergence) / 4
    let pressure = (pL + pR + pB + pT - div) * 0.25;
    
    textureStore(output, id.xy, vec4<f32>(pressure, 0.0, 0.0, 0.0));
}
