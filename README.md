...
- **Pen Input Window**: Pop out an input surface onto a pen display or tablet screen (Display → Pen Input Window); the paint stays on your main monitor at its frame rate, with a light mirror and the brush ring under the pen — and, in the desktop app, the mouse keeps working in the main window while the pen is down, and on Windows it picks up where it left off after the pen instead of starting from the tablet
