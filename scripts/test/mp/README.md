...
| `mp-phone-e2e.js` | **The phone brush end to end**: one headless Chrome runs this working tree's app, a second emulates an iPhone that opens the room link, is sent to `phone/` and paints with touch — against a local relay (`npx wrangler dev --port 8787`). Checks the "Paint from your phone" door and dialog (room, QR, no clipboard), the redirect, `pad-info` (canvas size, flow model, the brush and how it walks), the pad layout upright and sideways, dye landing on the computer as much as and where the same stroke painted there lands (fixed and random colours, time-matched), the phone cursor mark, Take turns and Call and return from the phone, Clear, the computer reloading (host again, phone re-counted), leaving, and `?full=1`. `SHOTS=<dir>` saves screenshots. ~30 s, 50 checks. |
