...
var marks = [];
function mark(tag){ var s = T.snapshotState({dyeOnly:true}); marks.push({tag:tag, dye:s.dye.hash, mean:+s.dye.mean.toFixed(8)}); }
return T.freeze().then(setup).then(function(){
  mark("post-setup");
  T.stroke({});
  mark("post-stroke-preframe");
  var chain = Promise.resolve();
  [1,2,3,5,10,20,40,60].forEach(function(n, i, arr){
    var prev = i===0?0:arr[i-1];
    chain = chain.then(function(){ return T.step(n-prev); }).then(function(){ mark("frame-"+n); });
  });
  return chain;
}).then(function(){
  T.unseed(); T.thaw();
