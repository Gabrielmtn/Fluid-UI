        ...
        // DEBUG: Track what changes when pointer leaves the canvas area (Electron bug)
        canvas.addEventListener('pointerleave', function() {
            console.log('[POINTER-LEAVE]',
                'canvas:', canvas.width, 'x', canvas.height,
                'style:', canvas.style.width, canvas.style.height,
                'wrapper:', canvasWrapper.clientWidth, 'x', canvasWrapper.clientHeight,
                'drawBuf:', gl.drawingBufferWidth, 'x', gl.drawingBufferHeight,
                'DPR:', window.devicePixelRatio,
                'fpsCap:', window.fpsCap,
                'fpsCapMode:', window.__fpsCapMode,
                'actualFPS:', window.__stats ? window.__stats.fps : '?');
        });
            ...
            try {
                ...
                if (remote && remote.screen) {
                    ...
                    if (display && display.displayFrequency > 0) {
                        ...
                        window.__displayHz = displayHz;

                        console.log('[Display Hz] Electron API: ' + displayHz + 'Hz');
        ...
        if (!detectDisplayHzElectron()) {
            ...
            console.log('[Display Hz] Electron API unavailable, using rAF fallback');
        ...
        function update() {
            ...
            if (canvas.width !== targetWidth || canvas.height !== targetHeight || window.needsFramebufferReinit) {
                ...
                console.log('[RESIZE]', 'canvas:', canvas.width, 'x', canvas.height, '→ target:', targetWidth, 'x', targetHeight, 'reinitFlag:', window.needsFramebufferReinit, 'drawBuf:', gl.drawingBufferWidth, 'x', gl.drawingBufferHeight, 'DPR:', window.devicePixelRatio);
