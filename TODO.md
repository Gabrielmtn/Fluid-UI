...
- [ ] **1a-2. Curl-noise micro-swirl in the dye advection pass (artistic option)** — near-free painterly texture.
  - Add a small time-animated curl-noise offset to the velocity *sampled inside the dye advection shader only* — never written back to the velocity texture. Zero passes, zero storage, feedback-safe (still bilinear resampling, gain ≤ 1).
  - Expose as a brush/material parameter (fits Material modes). Ref: Bridson curl-noise (repo: https://github.com/xt0x/curl-noise).
