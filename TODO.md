...
- [x] Collect real data — DONE 2026-07-09 across four field rounds (worst-list → summary → Event Timing → LookWatchdog); drove the compositor isolation, collapse-animation fix, cadence-honest metrics, 144Hz unlock, and the effect-shedding opt-in.
...
**Principle adopted 2026-07-09 (Gabriel): quality machinery (governor/battery/boot-ascent) may change FPS and fidelity, NEVER the aesthetic.**
RESOLVED the "ridges pop off" mystery (LookWatchdog): the governor — actually enabled despite the UI reading off (settings-hydration mismatch, checkboxes now re-synced post-hydration) — hit L5 under 144Hz load and flipped the fx gate, killing sharpen+micro-detail+sunrays+MacCormack in one stroke. Effect shedding is now OPT-IN ("Effect Shedding" checkbox in stats panel, default off, persisted): the ladder trades iterations+resolution only, unless the user explicitly allows the last rung. LookWatchdog stays in the build as the standing enforcement instrument for this principle. First application: sharpen kernel normalized to 2048-reference texels (resolution changes no longer alter the look) + "Ridges" slider makes the boot-ascent's coarse-emboss accident a deliberate effect + boot ascent now respects the persisted governor-off setting (settings-hydration race fixed in bootFactors). Micro-detail clarity kernel normalized 2026-07-09 (8783529) — all post-FX kernels now resolution-independent. Last remaining (minor, inherent): MacCormack ringing scale tracks dye-texel size; only visible if resolution tiers are allowed + shedding opted in.
...
- [ ] Focus
- [ ] Mutate Shader (wiring cleanup DONE 2026-07-09: flat 200ms delay removed, engine check retries instead of warn-and-die; content/UX audit still open)
- [ ] Audio
- [ ] Branding
- [ ] Layers
- [ ] Animations
