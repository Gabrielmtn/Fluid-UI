...
- [ ] **🐛 REGRESSION (reported 2026-07-13, FIX FIRST): path layers no longer respect their own color** — even with the brush-color checkbox off. Prime suspect: the brush fixes removed `resolveArmColor`'s `multiplier < 2` gate, so arm 0's color mode now applies at 1x — and path layers splat through `multiSplat`/`applyMultiSplatWith`, whose fallbackColor (the path's configured color) gets OVERRIDDEN by arm 0's non-'main' mode. Likely fix: programmatic splat sources (path layers, audio scenes, animations, replay?) should bypass arm-color resolution — add a color-override flag to multiSplat/applyMultiSplatWith (or set arm resolution to apply only to pointer-driven strokes). Audit ALL applyMultiSplatWith callers (24-path-layers, 30-audio-scenes, 04c/d/e animations, 05d replay) for the same exposure while there.
