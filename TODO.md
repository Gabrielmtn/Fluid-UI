...
- [ ] Open question from cadence data: gapMedian 16.7ms (≈60Hz presentation) while fpsCap=144 — if Gabriel's display is actually 144Hz, Electron is compositing at 60 and finding out why is a real smoothness lever (check Windows display settings / Electron frame limiter / power profile). If the display is 60Hz, ignore (cap>display is just uncapped; note simFps reads ~cap in that mode due to snap accounting — metric quirk, documented here).
